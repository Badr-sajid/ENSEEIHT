...
type info =
  ...
  (* Information associée à une valeur de type enumere : son nom, son type *)
  | InfoEnum of string * typ
...
(* Modifie le type si c'est une InfoVar ou InfoEnum, ne fait rien sinon *)
...
(* Recherche les informations d'une fonction dans la tds globale *)
(* Si la fonction n'est pas présente dans la tds de plus bas niveau *)
(* la recherche est effectuée dans sa table mère et ainsi de suite *)
(* jusqu'à trouver (ou pas) la fonction *)
(* Dans le cas où la fonction est trouvée on l'ajoute à la liste des fonction de même nom *)
val chercherGlobalementFunction : tds -> string -> info_ast list
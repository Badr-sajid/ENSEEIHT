...
public class IncrMesVolatile {
    ...
    static void lancer(int nbA, Incrémenteur r) {
        ...
        for (int i = 0; i < nbA ; i++) {
            try {
                IncrMesVolatile.activités[i] = new Thread(r,"t"+i);
            }
            catch (Exception e)
            {
                System.out.println(e);
            }
            IncrMesVolatile.activités[i].start();
        }
    }

    static void finir() {
        // Attente de la terminaison des différents threads lancés
        for (int i = 0; i < IncrMesVolatile.activités.length ; i++) {
            try {
                IncrMesVolatile.activités[i].join();
            }
    ...
    public static void main(String[] args) {
        ...
        // Idée = retenir une moyenne des dernières mesures.
            ...
            for (long li = 0; li < nbActivités*IncrMesVolatile.NB_IT; li++) {
                for (long j = 1; j<=IncrMesVolatile.NB_IT_INTERNES; j++) {
                    IncrMesVolatile.cpt=IncrMesVolatile.cpt+j/j;
                    // j/j pour tenter de déjouer les optimisations du compilateur
                }
                try {
                    Thread.sleep(IncrMesVolatile.Attente_ms, IncrMesVolatile.Attente_nano);
...
class IncrémenteurNonSync implements Incrémenteur {
     ...
     */
    public void incr() {
        for (long i = 0L; i < IncrMesVolatile.NB_IT; i++) {
    ...
    public void run() {
        ...
        this.incr();
        System.out.println(IncrMesVolatile.cpt);
    }
}

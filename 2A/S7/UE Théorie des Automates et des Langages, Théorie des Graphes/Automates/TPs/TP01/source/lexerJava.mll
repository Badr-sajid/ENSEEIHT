...
let underscores = '_'
let binary = '0' | '1'
let digit = chiffre ((underscores chiffre) | chiffre)*
...
(* Nombres entiers : A COMPLETER *)
  | ('0' | (['1' - '9'] ((underscores chiffre) | chiffre)* ))  as texte { (ENTIER (int_of_string texte)) }
  | ('0' ('x' | 'X') hexadecimal ((underscores hexadecimal) | hexadecimal)*) as texte   { (HEXADECIMAL (int_of_string texte)) }
  | ('0' ?underscores octal ((underscores octal) | octal)*)   as texte   { (OCTAL (int_of_string texte)) }
  | (("0b" | "0B") binary ((underscores binary) | binary)*) as texte   { (BINARY (int_of_string texte)) }

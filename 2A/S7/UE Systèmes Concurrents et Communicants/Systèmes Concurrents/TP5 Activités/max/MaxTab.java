...
// pool fork-join
    ...
    protected Integer compute() {
        int taille = début-fin;
        // si la tâche est trop grosse, on la décompose en 2
// ********* A compléter
        if (taille > seuil) {
        	TraiterFragment t1 = new TraiterFragment(tableau, début, Math.round((fin+début)/2));
        	TraiterFragment t2 = new TraiterFragment(tableau, Math.round((fin+début)/2), fin);
        	t1.fork();
        	t2.fork();
        	max = Math.max(t1.join(), t2.join());
		} else {
			for (int i = début; i < fin; i++) {
	        	max = Math.max(max, this.tableau[i]);
	        }
		}
        return max;
    }
...
// ********* A compléter
        for (Future<Integer> future : résultats) {

...
function Lagrangien_Augmente(algo,fonc::Function,contrainte::Function,gradfonc::Function,
    ...
    while (true)
        ...
        Hess_x_Lx(x) = hessfonc(x) + lambda'*hess_contrainte(x) + mu*hess_contrainte(x)*contrainte(x) +
            mu*grad_contrainte(x)*grad_contrainte(x)'

        # Calcul de xk 
        if algo == "newton"
          option = [itermax,epsilon,tol]
          xk, ~ = Algorithme_De_Newton(Lx,grad_x_Lx,Hess_x_Lx,xk,option)
        elseif algo == "cauchy" || algo == "gct"
          option = [10,0.5,2,0.25,0.75,2,1000,epsilon,tol]
          xk, ~ = Regions_De_Confiance(algo,Lx,grad_x_Lx,Hess_x_Lx,xk,[])
        end

        " Test convergence globale"
        gradL(x,l) = gradfonc(x) + l'*grad_contrainte(xk)
        if norm(gradL(xk,lambda)) <= max(epsilon, tol*norm(gradL(x0,lambda0)))
          flag = 0
          break
        ...
        elseif iter == itermax
          flag = 1
          break
        else # pas de convergence globale

...
// ********* A compléter
        if (taille > TF.seuil) {
        	TraiterFragment t1 = new TraiterFragment(tableau, début, Math.round((début+fin)/2));
        	TraiterFragment t2 = new TraiterFragment(tableau, Math.round((début+fin)/2), fin);
        	t1.fork();
        	t2.fork();
        	resTri = TF.fusion(t1.join(), t2.join());
		} else {
			return TF.traiterTronçon(tableau, début, fin) ;
		}
    	return resTri;
...
// ********* A compléter
        ...
        for (Future<int[]> future : résultats) {
		resTri.add(future.get());
	}

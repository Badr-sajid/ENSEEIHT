...
public class IncrMesSynchronized {
    ...
    static void lancer(int nbA, Incrémenteur r) {
        ...
        for (int i = 0; i < nbA ; i++) {
            try {
                IncrMesSynchronized.activités[i] = new Thread(r,"t"+i);
            }
            catch (Exception e)
            {
                System.out.println(e);
            }
            IncrMesSynchronized.activités[i].start();
        }
    }

    static void finir() {
        // Attente de la terminaison des différents threads lancés
        for (int i = 0; i < IncrMesSynchronized.activités.length ; i++) {
            try {
                IncrMesSynchronized.activités[i].join();
            }
    ...
    public static void main(String[] args) {
        ...
        // Idée = retenir une moyenne des dernières mesures.
            ...
            for (long li = 0; li < nbActivités*IncrMesSynchronized.NB_IT; li++) {
                for (long j = 1; j<=IncrMesSynchronized.NB_IT_INTERNES; j++) {
                    IncrMesSynchronized.cpt=IncrMesSynchronized.cpt+j/j;
                    // j/j pour tenter de déjouer les optimisations du compilateur
                }
                try {
                    Thread.sleep(IncrMesSynchronized.Attente_ms, IncrMesSynchronized.Attente_nano);
...
class IncrémenteurNonSync implements Incrémenteur {
     ...
     */
    public void incr() {
        for (long i = 0L; i < IncrMesSynchronized.NB_IT; i++) {
    ...
    public void run() {
        ...
        this.incr();
        System.out.println(IncrMesSynchronized.cpt);    }
}

...
public class IncrMesSynchronized1 {
    ...
    static void lancer(int nbA, Incrémenteur r) {
        ...
        for (int i = 0; i < nbA ; i++) {
            try {
                IncrMesSynchronized1.activités[i] = new Thread(r,"t"+i);
            }
            catch (Exception e)
            {
                System.out.println(e);
            }
            IncrMesSynchronized1.activités[i].start();
        }
    }

    static void finir() {
        // Attente de la terminaison des différents threads lancés
        for (int i = 0; i < IncrMesSynchronized1.activités.length ; i++) {
            try {
                IncrMesSynchronized1.activités[i].join();
            }
    ...
    public static void main(String[] args) {
        ...
        // Idée = retenir une moyenne des dernières mesures.
            ...
            for (long li = 0; li < nbActivités*IncrMesSynchronized1.NB_IT; li++) {
                for (long j = 1; j<=IncrMesSynchronized1.NB_IT_INTERNES; j++) {
                    IncrMesSynchronized1.cpt=IncrMesSynchronized1.cpt+j/j;
                    // j/j pour tenter de déjouer les optimisations du compilateur
                }
                try {
                    Thread.sleep(IncrMesSynchronized1.Attente_ms, IncrMesSynchronized1.Attente_nano);
...
class IncrémenteurNonSync implements Incrémenteur {
     ...
     */
    public void incr() {
        for (long i = 0L; i < IncrMesSynchronized1.NB_IT; i++) {
    ...
    public void run() {
        ...
        this.incr();
        System.out.println(IncrMesSynchronized1.cpt);
    }
}

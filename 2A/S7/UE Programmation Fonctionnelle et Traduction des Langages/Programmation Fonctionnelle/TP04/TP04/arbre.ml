...
let rec parcours (Noeud (b, lb)) = 
   match lb with
   | []   -> []
   | t::q -> let (br,ar) = t in let (Noeud (bar, _)) = ar in
                           if bar then ([br]::(List.map (fun list -> br::list) (parcours ar))) @ (parcours (Noeud (b, q)))
                           else (List.map (fun list -> br::list) (parcours ar)) @ (parcours (Noeud (b, q)))

...
procedure Evaluer_Alea is
    ...
    is
        ...
        CAPACITE: constant Integer := 100;   -- la capacité du tableau
        ...
        type Tableau is
            record
                ...
                T_Taille   : Integer;         --{ Taille in [0..CAPACITE] }
            end record;
        Tab : Tableau;
    begin
        Tab := ( (others => 0), Borne);
        ...
        for i in 1..Taille loop
            ...
            Tab.Elements(Nombre):= (Tab.Elements(Nombre)+1);
        end loop;
        min := tab.Elements(1);
        max := tab.Elements(1);
        for i in 1..Borne loop
            if tab.Elements(i) > max then
                max := tab.Elements(i);
            elsif tab.Elements(i) < min then
                min := tab.Elements(i);
            end if;
        end loop;

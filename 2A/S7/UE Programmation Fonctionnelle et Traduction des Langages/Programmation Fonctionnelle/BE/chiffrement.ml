...
struct
  ...
  let get_branche b (Noeud l) = List.assoc_opt b l
 ...
 *)
  ...
  (*
    let rec aux ls a =
      match a with
      | Lettre _ -> (dechiffrer ls arbre)::arbre_to_list a
      | Noeud (la) -> List.map (aux [] (List.map (fun l -> let (b,ar) = l in (aux (b::ls) ar)) la))
      
      
      (*(match la with
                      | [(b,ar)] -> [(aux (b::ls) ar)]
                      | t::q -> let (b,ar) = t in 
                        (List.map (aux (b::ls) ar) t)@(aux ls Noeud(q)))*)
      
      
      (*List.map (fun l -> let (b,ar ) = l in (aux (b::ls) ar)) la*)
    in aux [] arbre*)
...
struct
  ...
  let dechiffrer code arbre = 
    let codedecompose = DRCode.decompose code in 
    let symboledonne = Arbre.dechiffrer codedecompose arbre in
    DRDonnee.recompose symboledonne
  ...
  (* chiffrer : donnee -> (symbole_code,symbole_donnee) arbre_chiffrement -> code
     ...
     Exception DonneeNonValide : si la donnéee ne peut pas être chiffrée avec l'arbre de chiffrement 
   *)
  let chiffrer donne arbre = 
    let donnedecompose = DRDonnee.decompose donne in 
    let symbolecode = Arbre.chiffrer donnedecompose arbre in
    DRCode.recompose symbolecode

...
let afficher_globale tds =
  ...
  let modifier_type_info t i =
    ...
    | InfoEnum (n,_) -> i:= InfoEnum(n,t)
    | _ -> failwith "Appel modifier_type_info pas sur un InfoVar ni sur un InfoEnum"
...
let rec chercherGlobalementFunction tds nom =
  match tds with
  | Nulle -> []
  | Courante (m,c) ->
    begin
      match find_all c nom with
      | [] -> chercherGlobalementFunction m nom
      | i -> i@(chercherGlobalementFunction m nom)
    end

...
let padovan n = 
   let rec aux p up up_1 up_2 = 
      if p = n then up
      else aux (p+1) (up_1 + up_2) up up_1
   in match n with
      | 0 -> 0
      | 1 -> 0
      | 2 -> 1
...
let estPremier n = let rec aux i b =
      if i = n then b
      else if (not b) then b
      else aux (i+1) ((n mod i)!=0)
   in match n with 
   | 1 -> false
   | _ -> aux 2 true
